...
export function Button({
  ...
  const baseClasses =
    "inline-flex items-center justify-center font-semibold transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--app-accent)] disabled:opacity-50 disabled:pointer-events-none shadow-md backdrop-blur-md font-sans tracking-wide";

  const variantClasses = {
    primary:
      "bg-gradient-to-r from-[#0052FF] via-[#00C6FF] to-[#0052FF] text-white border border-transparent hover:shadow-[0_0_16px_2px_#00C6FF] hover:scale-[1.04]",
    secondary:
      "bg-[var(--app-gray)] bg-opacity-80 text-[var(--app-foreground)] border border-[var(--app-accent)] hover:bg-[var(--app-accent-light)] hover:shadow-[0_0_12px_1px_#0052FF] hover:scale-[1.04]",
    outline:
      "border border-[var(--app-accent)] bg-gradient-to-r from-[#232a3a] to-[#1a1f2b] text-[var(--app-accent)] hover:bg-[var(--app-accent-light)] hover:shadow-[0_0_12px_1px_#00C6FF] hover:scale-[1.04]",
    ghost:
      "bg-transparent text-[var(--app-foreground-muted)] hover:bg-[var(--app-accent-light)] hover:text-[var(--app-accent)] hover:scale-[1.04]",
  };
...
function Card({
  ...
  return (
    <div
      className={`bg-gradient-to-br from-[#232a3a] via-[#1a1f2b] to-[#232a3a] rounded-3xl border border-[var(--app-accent)] shadow-lg backdrop-blur-xl overflow-hidden transition-all duration-300 hover:shadow-[0_0_32px_4px_#00C6FF] hover:-translate-y-[2px] ${className} ${onClick ? "cursor-pointer" : ""}`}
      onClick={onClick}
    ...
    >
      {title && (
        <div className="px-8 py-5 border-b border-[var(--app-accent)] bg-gradient-to-r from-[#0052FF22] to-[#00C6FF11]">
          <h3 className="text-2xl font-semibold text-[var(--app-foreground)] tracking-wide drop-shadow-glow font-sans">
            {title}
          </h3>
        </div>
      )}
      <div className="p-8 space-y-4 font-sans">{children}</div>
    </div>
  );
}
...
type IconName = "heart" | "star" | "check" | "plus" | "arrow-right" | "trash" | "edit" | "settings" | "bell" | "user" | "search" | "moon" | "sun";
...
function DashboardCard({ onExplore }: { onExplore: () => void }) {
  ...
  return (
    <Card>
      <nav className="flex items-center justify-between px-2 py-2 mb-2">
        <div className="text-3xl font-extrabold tracking-wide bg-gradient-to-r from-[#00C6FF] to-[#0052FF] bg-clip-text text-transparent drop-shadow-glow font-sans">SoulLedger</div>
      ...
      <div className="space-y-7 animate-fade-in font-sans">
        ...
        <div className="grid grid-cols-3 gap-5 text-center">
          <div className="p-5 rounded-2xl bg-gradient-to-br from-[#0052FF22] to-[#00C6FF11] border border-[var(--app-accent)] shadow-md">
        ...
        {highlight && (
          <div className="p-5 rounded-2xl bg-gradient-to-r from-[#00C6FF22] to-[#0052FF11] border border-[var(--app-accent)] shadow-md animate-fade-in">
